...
from src.utils import load_all_data, load_target_data
...
def cv_train(name, model_id, args, run):
    ...
    for _ in range(args.n_cv):
        ...
        _, acc_train, acc_val = trainer.fit(
            model, dl_train_all, dl_val_all, verbose=True
        )
...
if args.train:
    ...
    for i in range(args.n_cv):
        ...
        _, acc_train, acc_val = trainer.fit(
            m1, dl_train_target, dl_val_target, verbose=True
        )
    ...
    for i in range(args.n_cv):
        ...
        m9 = Siamese(auxiliary=le_net, target=tail, softmax=False, strategy="sum")
        trainer = Trainer(
            nb_epochs=args.nb_epochs - 5, run="pretrained_tail_headTraining"
        )
...
if args.train:
    ...
    for name, test_acc in zip(model_names, test_accs):
        print(
            "{:30s}: Test accuracy: {:1.2f} ({:2.2f})".format(
                name, np.array(test_acc).mean(), np.array(test_acc).std()
            )
        )
else:
    # Evaluating models
    ...
    for _ in range(args.n_cv):
        ...
        test_acc.append(
            trainer.test(models[0], dl_test, test_verbose=False, return_acc=True)
        )
    print(
        "{:26s}: Test accuracy: {:1.2f} ({:2.2f})".format(
            model_names[0], np.array(test_acc).mean(), np.array(test_acc).std()
        )
    )
    ...
    for i, m in enumerate(models[1:]):
        ...
        for _ in range(args.n_cv):
            ...
            test_acc.append(
                trainer.test(m, dl_test, test_verbose=False, return_acc=True)
            )
        print(
            "{:26s}: Test accuracy: {:1.2f} ({:2.2f})".format(
                model_names[i + 1], np.array(test_acc).mean(), np.array(test_acc).std()
            )
        )
