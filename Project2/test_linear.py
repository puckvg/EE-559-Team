...
class TestLinear(TestModule):
    ...
    def _forward_batch(self, batch_size, in_dim, out_dim):
        # Generate random test data
    ...
    def test_random_forward_batch(self):
        for _ in range(n_tests):
            batch_size = random.randint(1, max_batch_size)
            in_dim = random.randint(1, max_dim)
            out_dim = random.randint(1, max_dim)
            self._forward_batch(batch_size, in_dim, out_dim)
    
    def _backward_batch(self, batch_size, in_dim, out_dim):
        mod_ours, mod_theirs = self._forward(in_dim, out_dim)
    ...
    def test_small_backward_batch(self):
        for _ in range(n_tests):
            in_dim = 3
            out_dim = 2
            batch_size = 2
            self._backward_batch(batch_size, in_dim, out_dim)
    
    def test_random_backward_batch(self):
        for _ in range(n_tests):
            in_dim = random.randint(1, max_dim)
            out_dim = random.randint(1, max_dim)
            batch_size = random.randint(1, max_batch_size)
...
testlin = TestLinear()
testlin.test_random_backward_batch()
